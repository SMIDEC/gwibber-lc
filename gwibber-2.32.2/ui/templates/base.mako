...
<%def name="liked_by(data)">
  <p class="likes">${_("%s user(s) liked this" % data.liked_by)}</p> 
</%def>
...
<%def name="buttons(data)">
  <div class="buttonitem">
    <a href="gwibber:/menu?msg=${data['id']}"><img width="16px" src="${resources.icon('system-run')}" title=${_("Menu")} /></a> 
  </div>
  <div class="buttonitem">
    %if not data["sender"].get("is_me", 0):
      % if data.get("private", 0):
        % if "recipient" in data:
          <a href="gwibber:/private?msg=${data['id']}"><img width="16px" src="${resources.icon('mail-reply-sender')}" title=${_("Reply")} /></a> 
        % endif
      % else:
        <a href="gwibber:/reply?msg=${data['id']}"><img width="16px" src="${resources.icon('mail-reply-sender')}" title=${_("Reply")} /></a>
      % endif
    % endif
  </div>
</%def>
...
<%def name="fold(data, ops=['location', 'liked_by', 'comments', 'extended_text', 'images'])">
  <div class="fold">
    % for o in ops:
      % if data.get(o, 0):
        ${getattr(self, o)(data)}
